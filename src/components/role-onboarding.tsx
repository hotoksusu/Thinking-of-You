...
function Scene({scene,role,onAnswer}:{scene:Step["scene"];role:Role;onAnswer:()=>void}){
  ...
  if(scene==="question")return <div className="mt-6 rounded-[28px] bg-white p-5 shadow-[0_16px_36px_rgba(55,72,55,.1)]"><div className="mb-5 flex items-center gap-4 rounded-[22px] bg-[#F2F7EF] p-4"><Image src="/brand/oneul-anbu-icon.png" alt="" width={56} height={56} className="size-14 object-contain"/><p className="text-xl font-black leading-8 text-[#245C3B]">편한 답을<br/>하나 눌러주세요.</p></div><div className="grid gap-3">{["괜찮아요","조금 불편해요","오늘은 답하지 않을게요"].map(x=><button key={x} onClick={()=>{setAnswer(x);try{localStorage.setItem("parentDemoAnswer",x)}catch{};window.dispatchEvent(new CustomEvent("todayanbu:analytics",{detail:{name:"parent_demo_completed"}}));onAnswer()}} className={`min-h-[72px] rounded-2xl border-2 px-5 text-left text-xl font-black shadow-sm ${answer===x?"border-[#2F6B46] bg-[#EAF3E5]":"border-[#AFC0AC] bg-white"}`}>{answer===x?<Check className="mr-2 inline"/>:null}{x}</button>)}</div><p className="mt-5 text-lg font-black text-[#4A5D53]">잘못된 답은 없습니다.</p></div>;
