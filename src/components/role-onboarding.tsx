...
import { ArrowLeft, Check, Coffee, HelpCircle, Home, Phone, UsersRound, X } from "lucide-react";
...
export function RoleOnboarding({role,initialStep=1}:{role:Role;initialStep?:number}){
  ...
  return <main className="flex min-h-screen flex-col bg-[#FFF9F0] text-[#20302C]">
    ...
    <div className="mx-auto flex w-full max-w-[860px] flex-1 flex-col px-5 py-8 sm:py-12"><div className="flex gap-2" aria-label={`${steps.length}단계 중 ${step}단계`}>{steps.map((_,i)=><span key={i} className={`h-2 flex-1 rounded-full ${i<step?"bg-[#6F9471]":"bg-[#DDE5DA]"}`}/>)}</div>
      <section key={step} className="fade-in flex flex-1 flex-col justify-center py-8"><p className="text-sm font-black text-[#D95C24]">{step}단계</p><h1 className="mt-3 max-w-[740px] text-[clamp(2rem,6vw,3.8rem)] font-black leading-[1.16] tracking-[-.035em]">{current.title}</h1><p className="mt-4 text-lg font-bold leading-8 text-[#5E6C66] sm:text-xl">{current.text}</p><Scene scene={current.scene} role={role}/></section>
...
function Scene({scene,role}:{scene:Step["scene"];role:Role}){const [answer,setAnswer]=useState("");if(scene==="rest")return <div className="mt-8 flex min-h-48 items-center justify-center rounded-[28px] bg-[#EAF3E5] text-[#315B3D]"><Coffee size={72}/></div>;if(scene==="empty")return <div className="mt-8 rounded-[28px] bg-white p-7 shadow-[0_16px_36px_rgba(55,72,55,.08)]"><p className="text-sm font-black text-[#6F9471]">오늘</p><p className="mt-3 text-2xl font-black">오늘은 하실 일이 없어요.</p><p className="mt-2 font-bold text-[#68756F]">편안한 하루 보내세요.</p></div>;if(scene==="question")return <div className="mt-8 rounded-[28px] bg-white p-6 shadow-[0_16px_36px_rgba(55,72,55,.08)]"><p className="text-xs font-black text-[#D95C24]">체험용 화면</p><p className="mt-3 text-2xl font-black">오늘은 평소보다 조금 피곤하신가요?</p><div className="mt-5 grid gap-3">{["괜찮아요","조금 피곤해요","오늘은 답하지 않을게요"].map(x=><button key={x} onClick={()=>setAnswer(x)} className={`min-h-16 rounded-2xl border-2 px-5 text-left text-lg font-black ${answer===x?"border-[#2F6B46] bg-[#EAF3E5]":"border-[#DDE4DA] bg-white"}`}>{answer===x?<Check className="mr-2 inline"/>:null}{x}</button>)}</div>{answer?<p role="status" className="mt-4 font-black text-[#2F6B46]">답변이 바로 저장되었습니다.</p>:null}</div>;if(scene==="family")return <div className="mt-8 rounded-[28px] bg-white p-6 shadow-[0_16px_36px_rgba(55,72,55,.08)]"><p className="text-xs font-black text-[#D95C24]">가족 화면 · 데모</p><p className="mt-3 text-2xl font-black">평소보다 활동이 줄었습니다.</p><div className="mt-5 space-y-3"><Line icon={UsersRound} label="오늘 상태" value="확인이 필요해요"/><Line icon={HelpCircle} label="변화 이유" value="최근 움직임 감소"/><Line icon={Phone} label="권장 행동" value="오늘 저녁에 전화해 보세요"/></div></div>;return <div className="mt-8 flex min-h-48 flex-col items-center justify-center rounded-[28px] bg-[#EAF3E5] text-center"><Check size={58} className="text-[#2F6B46]"/><p className="mt-4 text-2xl font-black">안내를 마쳤어요.</p>{role==="parent"?<Link href="/for-family" className="mt-3 font-black text-[#315B3D] underline underline-offset-4">가족은 어떻게 확인하나요?</Link>:<div className="mt-4 flex gap-4"><Link href="/how-it-works" className="font-black underline underline-offset-4">작동 방식</Link><Link href="/pricing" className="font-black underline underline-offset-4">요금</Link></div>}</div>}
