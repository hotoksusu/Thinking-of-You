...
export function AdminCareAnalytics() {
  ...
  const [events,setEvents]=useState<CareAnalyticsEvent[]>([]);
  useEffect(() => {
    const sync = () => {setState(loadCareState());setEvents(loadCareAnalyticsEvents())};
    sync();
    window.addEventListener("todayanbu:care-updated", sync);
    return () => window.removeEventListener("todayanbu:care-updated", sync);
  }, []);

  const metrics = useMemo(() => state ? calculateMetrics(state, events, period) : null, [state, events, period]);
  ...
  return <main className="min-h-screen bg-[#F2F5F3] p-5 text-[#18231F] lg:p-10"><div className="mx-auto max-w-[1240px]">
    ...
    <section className="mt-6 grid gap-5 lg:grid-cols-2"><article className="rounded-[24px] border border-[#DCE5E0] bg-white p-6"><p className="text-sm font-black text-[#315E50]">PATIENT ENGAGEMENT</p><h2 className="mt-1 text-2xl font-black">환자 참여</h2><div className="mt-5 grid grid-cols-2 gap-3"><Small label={`${periodLabel} 참여율`} value={`${metrics.participationRate}%`} /><Small label="2일 이상 연속 참여" value={`${metrics.consecutivePatients}명`} /></div><p className="mt-4 text-sm font-semibold leading-6 text-[#68766F]">참여율은 환자의 퇴원일 이후 선택 기간에 응답 가능한 환자-일을 분모로 계산합니다. Day 3·Day 7 유지율은 아래에서 별도로 확인합니다.</p></article><Distribution title={`${periodLabel} 조치 유형`} rows={metrics.actionCounts.map(([key, count]) => [actionLabels[key] || key, count])} /></section>

    <section className="mt-6 grid gap-5 lg:grid-cols-2"><article className="rounded-[24px] border bg-white p-6"><p className="text-sm font-black text-[#315E50]">PATIENT ENGAGEMENT</p><h2 className="mt-1 text-2xl font-black">Check-in Funnel & Quality</h2><div className="mt-5 grid grid-cols-2 gap-3"><Small label="Day 3 유지율" value={`${metrics.day3Retention}%`} /><Small label="Day 7 유지율" value={`${metrics.day7Retention}%`} /><Small label="완료율" value={`${metrics.checkinCompletionRate}%`} /><Small label="평균 완료시간" value={metrics.averageCheckinSeconds?`${metrics.averageCheckinSeconds}초`:"기록 없음"} /><Small label="Adaptive 질문 발생" value={`${metrics.adaptiveRate}%`} /><Small label="잘 모르겠어요" value={`${metrics.unsureRate}%`} /><Small label="3일 이상 동일 통증" value={`${metrics.repeatedAnswerRate}%`} /><Small label="가장 높은 이탈" value={metrics.topDropoff}/></div><p className="mt-4 text-xs font-semibold text-[#68766F]">응답 품질은 환자 평가가 아니라 질문과 UX 개선을 위한 내부 지표입니다.</p></article><article className="rounded-[24px] border bg-white p-6"><p className="text-sm font-black text-[#315E50]">SIGNAL FUNNEL</p><h2 className="mt-1 text-2xl font-black">Signal → Action</h2><div className="mt-5 grid gap-3"><Flow label="Care Signal 발생" value={`${metrics.signalCreated}건`} /><Flow label="의료진 확인" value={`${metrics.signalViewed}건`} /><Flow label="도움됨 평가" value={`${metrics.signalUseful}건`} /><Flow label="Care Action" value={`${metrics.actionCompleted}건`} /><Flow label="Follow-up 생성" value={`${metrics.followupCreated}건`} /></div></article></section>

    <section className="mt-6 rounded-[24px] border bg-white p-6"><p className="text-sm font-black text-[#315E50]">HOSPITAL WORKFLOW</p><h2 className="mt-1 text-2xl font-black">처리시간과 완료율</h2><div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4"><Small label="Signal → 최초 확인" value={metrics.averageFirstViewMinutes?`${metrics.averageFirstViewMinutes}분`:"기록 없음"}/><Small label="평균 Action 처리" value={metrics.averageActionMinutes?`${metrics.averageActionMinutes}분`:"기록 없음"}/><Small label="Signal → Action 전환" value={rate(metrics.actionCompleted,metrics.signalCreated)}/><Small label="Follow-up 완료율" value={rate(metrics.followCompleted,metrics.followLifecycle)}/></div></section>
...
function calculateMetrics(state: CareState, events:CareAnalyticsEvent[], period: Period) {
  ...
  const scopedEvents=events.filter(event=>inPeriod(event.timestamp)),starts=scopedEvents.filter(event=>event.eventType==="checkin_started"),completes=scopedEvents.filter(event=>event.eventType==="checkin_completed"),questions=scopedEvents.filter(event=>event.eventType==="checkin_question_answered"),durations=completes.map(event=>Number(event.metadata?.durationSeconds||0)).filter(Boolean),signalCreated=scopedEvents.filter(event=>event.eventType==="care_signal_created").length,signalViewed=scopedEvents.filter(event=>event.eventType==="care_signal_viewed"||event.eventType==="hospital_patient_opened").length,signalUseful=(state.signalFeedback||[]).filter(item=>item.feedback==="useful"&&inPeriod(item.submittedAt)).length,actionCompleted=scopedEvents.filter(event=>event.eventType==="care_action_completed").length,followupCreated=scopedEvents.filter(event=>event.eventType==="followup_created").length,startedIds=new Set(starts.map(event=>event.patientId)),completedIds=new Set(completes.map(event=>event.patientId));
  const retention=(day:number)=>{const eligible=managedPatients.filter(patient=>shiftDate(patient.dischargeDate,day-1)<=TODAY);const retained=eligible.filter(patient=>state.checkIns.filter(check=>check.patientId===patient.id&&check.date>=patient.dischargeDate&&check.date<=shiftDate(patient.dischargeDate,day-1)).length>=Math.ceil(day*.6));return eligible.length?Math.round(retained.length/eligible.length*100):0};
  const repeated=managedPatients.filter(patient=>{const scores=state.checkIns.filter(check=>check.patientId===patient.id).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,3).map(check=>check.painScore??check.pain);return scores.length===3&&new Set(scores).size===1}).length,dropoffs=Object.entries(questions.reduce<Record<string,number>>((acc,event)=>{const q=String(event.metadata?.question||"알 수 없음");acc[q]=(acc[q]||0)+1;return acc},{})).sort((a,b)=>a[1]-b[1]);
  const viewTimes=state.careTasks?.map(task=>task.firstReviewedAt?(Date.parse(task.firstReviewedAt)-Date.parse(task.createdAt))/60000:0).filter(value=>value>0)||[],actionTimes=(state.careActions||[]).map(action=>action.startedAt&&action.completedAt?(Date.parse(action.completedAt)-Date.parse(action.startedAt))/60000:0).filter(value=>value>0);
  return { managed: managedPatients.length, todayChecked: todayCheckedIds.size, participationRate: eligiblePatientDays ? Math.round(respondedPatientDays / eligiblePatientDays * 100) : 0, respondedPatientDays, eligiblePatientDays, attention: attentionIds.size, actions, followScheduled, followCompleted, followLifecycle, todayNoResponse: Math.max(0, todayEligible.length - todayCheckedIds.size), actedAttentionPatients, consecutivePatients: managedPatients.filter(patient => hasConsecutiveDays(state.checkIns.filter(check => check.patientId === patient.id && inPeriod(check.checkInDate || check.date)).map(check => check.checkInDate || check.date))).length, actionCounts,day3Retention:retention(3),day7Retention:retention(7),checkinCompletionRate:startedIds.size?Math.round(completedIds.size/startedIds.size*100):0,averageCheckinSeconds:durations.length?Math.round(durations.reduce((a,b)=>a+b,0)/durations.length):0,adaptiveRate:completes.length?Math.round(completes.filter(event=>event.metadata?.adaptiveQuestion===true).length/completes.length*100):0,unsureRate:questions.length?Math.round(questions.filter(event=>event.metadata?.unsureSelected===true).length/questions.length*100):0,repeatedAnswerRate:managedPatients.length?Math.round(repeated/managedPatients.length*100):0,topDropoff:dropoffs[0]?.[0]||"기록 없음",signalCreated,signalViewed,signalUseful,actionCompleted,followupCreated,averageFirstViewMinutes:viewTimes.length?Math.round(viewTimes.reduce((a,b)=>a+b,0)/viewTimes.length):0,averageActionMinutes:actionTimes.length?Math.round(actionTimes.reduce((a,b)=>a+b,0)/actionTimes.length):0 };
}
