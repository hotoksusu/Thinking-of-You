...
const analysisItems = [
  { title: "응답 시간 변화", description: "평소보다 늦거나 빨라진 응답 흐름", icon: Clock3 },
  { title: "생활 루틴 변화", description: "식사, 안부 입력, 하루 흐름의 변화", icon: HeartHandshake },
  { title: "활동량 변화", description: "평소 대비 줄어든 움직임과 외출 신호", icon: Activity },
  { title: "반복 응답 누락", description: "하루 이상 반복되는 미응답 패턴", icon: MessageCircle },
  { title: "감정 표현 변화", description: "평소와 다른 컨디션 표현과 말투", icon: Smile },
];
...
const values = [
  {
    ...
    description: "반복 확인보다 중요한 변화 신호를 먼저 살펴볼 수 있게 돕습니다.",
    icon: Bell,
  },
  {
    title: "평소와 다른 변화를 놓치지 않습니다",
    description: "응답 시간, 생활 루틴, 활동 흐름의 차이를 한 화면에서 확인합니다.",
    icon: Activity,
  },
  {
    title: "가족이 같은 안심 정보를 공유합니다",
    description: "형제자매가 같은 리포트를 보고 함께 살펴볼 수 있습니다.",
    icon: Users,
  },
];
  ...
  return (
    <main className="min-h-screen bg-white text-[#1F2937]">
      ...
      <section className="mx-auto grid w-full max-w-[1120px] gap-14 px-5 pb-20 pt-10 sm:px-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:gap-16 lg:pb-28 lg:pt-16">
        <div className="fade-in max-w-[560px]">
          ...
          <p className="mt-5 max-w-[540px] text-base font-semibold leading-7 text-[#6B7280]">
            오늘안부는 안부 확인을 대신하는 서비스가 아니라, 평소와 다른 변화를 가족이 더 빨리
            알아차릴 수 있도록 돕는 AI 안심 서비스입니다.
          </p>
          <div className="mt-9 grid max-w-[420px] gap-3 sm:grid-cols-2">
            <a
              href="/app"
      ...
      <section className="mx-auto w-full max-w-[1120px] px-5 py-24 sm:px-8">
        <div className="max-w-[680px]">
          <p className="text-sm font-black text-[#2563EB]">리포트 결과</p>
          <h2 className="mt-4 text-4xl font-black tracking-normal">AI가 알려주는 것</h2>
        </div>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {aiNotices.map((notice, index) => (
            ...
            >
              ...
              <h3 className="mt-5 text-2xl font-black leading-8">{notice}</h3>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto grid w-full max-w-[1120px] gap-10 px-5 py-24 sm:px-8 lg:grid-cols-[0.86fr_1.14fr] lg:items-center">
        <div>
          <p className="text-sm font-black text-[#2563EB]">신뢰 설명</p>
          <h2 className="mt-4 text-4xl font-black tracking-normal">
            건강 진단이 아니라 가족을 위한 안심 신호입니다
          </h2>
        ...
        <div className="grid gap-4">
          {trustNotes.map((note) => (
            <div
              key={note}
              className="flex items-start gap-4 rounded-[24px] bg-[#F9FAFB] p-5"
            >
      ...
      <section className="mx-auto w-full max-w-[760px] px-5 py-24 text-center sm:px-8">
        <div className="rounded-[28px] bg-[#EFF6FF] p-8 sm:p-10">
          ...
          <p className="mx-auto mt-5 max-w-[520px] text-lg font-semibold leading-8 text-[#6B7280]">
            평소와 다른 신호를 더 빨리 알아차릴 수 있도록 오늘안부가 보조합니다.
          </p>
          <a
            href="/app"
...
function ChangeSignalCard() {
  return (
    <article className="warm-card rounded-[28px] bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      ...
      <p className="mt-5 text-sm font-black text-[#6B7280]">응답 시간 변화</p>
