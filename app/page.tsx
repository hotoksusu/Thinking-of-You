...
export default function LandingPage() {
  return <main className="min-h-screen bg-[#FFF9F0] text-[#20302C]">
    ...
    <section className="mx-auto grid min-h-[calc(100svh-84px)] max-w-[1240px] items-center gap-12 px-6 py-12 lg:grid-cols-[1.08fr_.92fr] lg:py-16">
      <div><p className="text-lg font-black text-[#D95C24]">부모님과 가족을 위한 오늘안부</p><h1 className="mt-5 max-w-[720px] text-[clamp(3.2rem,5.6vw,5.6rem)] font-black leading-[1.08] tracking-[-.045em]">매일 묻지 않아도,<br/>다른 날만 알려드립니다.</h1><p className="mt-7 max-w-[680px] text-[clamp(1.3rem,1.7vw,1.65rem)] font-bold leading-[1.6] text-[#4E6259]">부모님은 평소처럼 지내고,<br className="sm:hidden"/> 가족은 필요한 날만 확인하세요.</p><div className="mt-10 grid max-w-[720px] gap-4 sm:grid-cols-2"><RoleLink href="/for-parent" icon={UserRound} title="부모님 화면 보기" text=""/><RoleLink href="/for-family" icon={UsersRound} title="가족 화면 보기" text=""/></div></div>
...
function RoleLink({href,icon:Icon,title,text,large=false}:{href:string;icon:typeof UserRound;title:string;text:string;large?:boolean}){return <Link href={href} className={`group flex min-h-[112px] items-center gap-4 rounded-[24px] border-2 border-[#C9D8C5] bg-white p-6 text-left shadow-[0_12px_28px_rgba(45,70,50,.06)] hover:border-[#6F9471] ${large?"sm:min-h-36 sm:p-7":""}`}><span className="flex size-14 shrink-0 items-center justify-center rounded-2xl bg-[#EAF3E5] text-[#315B3D]"><Icon size={29}/></span><span className="flex-1"><strong className="block text-xl font-black sm:text-2xl">{title}</strong>{text&&<span className="mt-2 block text-lg font-bold text-[#65736C]">{text}</span>}</span><ArrowRight size={28} className="text-[#52725B] transition group-hover:translate-x-1"/></Link>}
